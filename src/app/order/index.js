...
export default function Order() {
  ...
  const [{ user }] = useStore();
  const { data : jsonData, setData: setJsonData, error : errorExcel , readExcelFile } = useExcelReader();
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      for(let col of jsonData){
        ...
        let resProducts = await authorizeAxiosInstance.post(`${URL_ROOT}/private/product/get-product-by-skus`, skus);
        ...
        if(resProducts.data. length < skus.length){
          toast.warning("Một số sản phẩm bạn không có quyền order! tại row: " + col.__rowNum__)
        }
  ...
  function validateOrder(order) {
    ...
    if (order.type === "By_Seller") {
        ...
        if (!order.phone) {
          ...
          if (!/^(\(\+1\)|\+1)?\s?\d{3}[-.\s]?\d{3}[-.\s]?\d{4}$/.test(phoneNumber)) {
            errors += "Phone must be a valid US number (e.g., +1 812-258-7306 or 812-258-7306). ";
            isValid = false;
          }
        }
        if (!order.street) {
...
const handlerSubmitImportExcel = async ()=>{
    try{
      await authorizeAxiosInstance.post(`${URL_ROOT}/private/order/create-order-by-file`, ordersImport);
  ...
  return (
    <Main title={"Order"}>
      <div>
        <OrderModal orders={ordersImport} showModal={showModal} handleClose={handleClose} handleSubmit={handlerSubmitImportExcel} />
        ...
        <div className="d-flex justify-content-between flex-wrap">
          ...
          <div className="flex-grow-1 me-3">
            <Form.Group>
              ...
              <Form.Control
                type="text"
                placeholder="Tìm kiếm bằng id hoặc extraId"
                name="search"
                value={search.search}
      ...
      <div>
        <OrderTableInfo orders={orders} orderKey={key} />
      </div>
